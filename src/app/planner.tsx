import { CalendarPlus, CheckCircle2, ListFilter, Save, ShoppingBasket, Shuffle, Trash2, X } from 'lucide-react-native';
...
function PlannerContent() {
  ...
  const handleDeletePlan = async (planId: string) => {
    ...
    try {
      ...
      Alert.alert('Could not delete plan', error instanceof Error ? error.message : 'Try again in a moment.');
    } finally {
  ...
  const handleGenerateShoppingList = async () => {
    if (!activePlan) {
      Alert.alert('Create a plan first', 'Generate a weekly plan before building a shopping list.');
      return;
    }
    ...
    try {
      ...
      Alert.alert('Could not build shopping list', error instanceof Error ? error.message : 'Try again in a moment.');
    } finally {
      setBuildingShoppingList(false);
    }
  };

  const handleSwapMeal = async (dayDate: string, mealId: string) => {
    setSwappingMealId(mealId);
    try {
      await swapMeal(dayDate, mealId);
    } catch (error) {
      Alert.alert('Could not randomize meal', error instanceof Error ? error.message : 'Try again in a moment.');
    } finally {
  ...
  return (
    <AppShell>
      <PageHeader
        ...
        action={
          <>
            <Button icon={activePlan ? Shuffle : CalendarPlus} onPress={handleGeneratePlan} disabled={generatingPlan}>
              {generatingPlan ? (activePlan ? 'Regenerating...' : 'Generating...') : activePlan ? 'Regenerate plan' : 'Generate plan'}
            </Button>
          </>
        }
      />
      ...
      <View style={styles.statsGrid}>
        ...
        <Card style={styles.shoppingCard}>
          ...
          <Button variant="secondary" icon={ShoppingBasket} onPress={handleGenerateShoppingList} disabled={buildingShoppingList}>
            {buildingShoppingList ? 'Building...' : 'Generate'}
          </Button>
        </Card>
      </View>
                    ...
                    <View key={meal.id} style={styles.mealSlot}>
                      ...
                      <View style={styles.mealActions}>
                        <Pressable
                          ...
                          accessibilityRole="button"
                          onPress={() => handleSwapMeal(day.date, meal.id)}
                          style={({ pressed }) => [styles.mealActionButton, pressed && styles.swapButtonPressed]}>
                          <Shuffle size={14} color={colors.primary} />
                          <Text style={styles.mealActionText}>{swappingMealId === meal.id ? 'Finding...' : 'Random'}</Text>
                        </Pressable>
                        <Pressable
      ...
      <View style={styles.templates}>
        ...
        {weeklyPlans.length ? (
          ...
          <Card tone="low">
            <Text style={styles.cardTitle}>No saved meal plans</Text>
